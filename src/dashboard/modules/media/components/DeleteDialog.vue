...
const title = props.count > 1 ? `${props.count} عنصر` : (props.name || 'هذا الملف')
</script>
    ...
    <div v-if="count" class="dialog-overlay" @click.self="emit('cancel')">
      <div class="dialog" role="alertdialog" aria-modal="true">
        ...
        <p class="dialog__text">
          هل أنت متأكد من حذف <strong>{{ title }}</strong>؟ سيتم حذف الملف من التخزين نهائيًا ولا يمكن التراجع.
        </p>
